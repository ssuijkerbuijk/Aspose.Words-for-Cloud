...
public class AddWaterMarkToDocumentExample {
	...
	public static void main(String[] args) {
		try {
            ...
            storageApi.PutCreate(fileName, "", storage, new File(AddWaterMarkToDocumentExample.class.getResource("/" + fileName).toURI()));

            //upload input file to 3rd party cloud storage
            storageApi.PutCreate(image, "", storage, new File(AddWaterMarkToDocumentExample.class.getResource("/" + image).toURI()));

            file = new File(AddWaterMarkToDocumentExample.class.getResource("/" + image).toURI());

            // invoke Aspose.Words Cloud SDK API to add watermark image in a word document
